...
// 📬 Query submission endpoint (NEW ROUTE)
app.post('/send-query', quoteRateLimiter, async (req, res) => {
    try {
        const data = req.body;
        ...
        // 4. Construct HTML email for Query
        const htmlContent = `
        <!DOCTYPE html>
        <html>
        <head>
          ...
          <style>
            ...
            .label { display: table-cell; width: 35%; padding: 8px 5px; font-weight: bold; color: #555555; border-bottom: 1px solid #f0f0f0; vertical-align: top; font-size: 14px; }
            .value { display: table-cell; width: 65%; padding: 8px 5px; color: #222222; border-bottom: 1px solid #f0f0f0; vertical-align: top; font-size: 14px; }
            .notes-box { background-color: #f8fafc; border-left: 4px solid #002244; padding: 15px; margin-top: 10px; font-style: italic; border-radius: 0 4px 4px 0; color: #475569; font-size: 14px; white-space: pre-wrap; }
            .footer { background-color: #f8fafc; text-align: center; padding: 15px; font-size: 11px; color: #64748b; border-top: 1px solid #e2e8f0; }
          </style>
        </head>
        <body>
          <div class="container">
            <div class="header">
              ...
              <p>SKYHAUL TRANSIT INC. — Customer Query Portal</p>
            </div>
            <div class="content">
              ...
              <div class="notes-box">${sanitizeString(data.message)}</div>

            </div>
            <div class="footer">
              This email was automatically generated by the Skyhaul Transit query system. Please click reply to respond directly to the sender.
            </div>
          </div>
        </body>
        </html>
        ...
        // 5. Send email
        const mailOptions = {
            from: `"Skyhaul Query" <${EMAIL_USER}>`,
            to: EMAIL_TO || EMAIL_USER,
            replyTo: data.email,
            subject: `New Query from: ${sanitizeString(data.name)}`,
            html: htmlContent
        };

        await transporter.sendMail(mailOptions);
        console.log("📨 Query notification email successfully sent to:", EMAIL_TO || EMAIL_USER);
